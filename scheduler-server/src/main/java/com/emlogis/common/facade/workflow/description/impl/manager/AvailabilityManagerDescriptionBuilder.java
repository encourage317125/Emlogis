...
@Local
@Default
@RequestDescriptionBuilderQualifier(value = "AVAILABILITY_REQUEST_MANAGER_DESCRIPTION")
@TransactionManagement(value = TransactionManagementType.CONTAINER)
@TransactionAttribute(TransactionAttributeType.REQUIRED)
public class AvailabilityManagerDescriptionBuilder implements DescriptionBuilder {
    ...
    public String build(WorkflowRequest request, DateTimeZone dtz, Locale locale) {
        ...
        switch (request.getAvailabilityRequestSubtype()) {
            case AvailcalUpdateParamsCDAvailDto: {
                AvailcalUpdateParamsCDAvailDto dto = fromJsonString(request.getData(),
                        AvailcalUpdateParamsCDAvailDto.class);
                selectedDates = dto.getSelectedDates();
                Boolean isAvailability = dto.getAction().equals(AVAILABLE_FOR_DAY) || dto.getAction().equals(AVAILABLE_FOR_TIMEFRAMES);
                String isAvailabilityMsg = isAvailability ? resourcesBundle.getMessage(locale.getLanguage(),
                        "description.available") : resourcesBundle.getMessage(locale.getLanguage(), "description.unavailable");
                builder.append(isAvailabilityMsg + " ");
                ...
                for (Long selectedDate : selectedDates) {
                    i++;
                    String dateStr = datesHelper.dateStr(dtz, selectedDate, locale);
                    builder.append(dateStr + " ");
                    for (AvailcalSimpleTimeFrame tf : dto.getTimeFrames()) {
                        k++;
                        Boolean isLastTimeFrame = dto.getTimeFrames().size() == 1 || (dto.getTimeFrames().size() - 1) == k;
                        String startTimeStr = timeStr(dtz, selectedDate + tf.getStartTime(), locale);
                        String endDateStr = timeStr(dtz, selectedDate + tf.getEndTime(), locale);
                        builder.append(startTimeStr + " - " + endDateStr + (isLastTimeFrame ? " / " : ", "));
                    }
                }
