...
//                {name: 'TEAM FLOAT ON', value:'TEAM_FLOAT_ON'}
            ...
            function createEmptyModel(employees) {
                $scope.employees = _.uniq(employees.data, function (item) {
                    return item['employeeSummaryDto'].id;
                });
            ...
            function setModel(options) {
                ...
                for (var i in opts) {
                    switch (opts[i]['scope']){
                        ...
                        case "Select":
                            for(var j=0; j < $scope.employees.length; j++){
                                if(_.contains(opts[i]['employeeIds'], $scope.employees[j]['employeeSummaryDto'].id)){
                                    $scope.employees[j]['options'][i] = true;
                                }
            ...
            $scope.change = function(emp, opt) {
                ...
                if(emp.options[opt.value]){
                    ids.push(
                        emp['employeeSummaryDto'].id
                    )
                } else {
                    if(option['scope'] == "All"){
                        for(var j=0; j < $scope.employees.length; j++){
                            ids.push(
                                $scope.employees[j]['employeeSummaryDto'].id
                            )
                        }
                    }
                    var index = ids.indexOf(emp['employeeSummaryDto'].id);
                    ids.splice(index, 1);
