...
public class ScheduleShiftSwapEligibilityClient {

    private final static Logger logger = Logger.getLogger(ScheduleShiftSwapEligibilityClient.class);
    ...
    public static void main(String[] args) {
    	ScheduleShiftSwapEligibilityClient client = new ScheduleShiftSwapEligibilityClient();
    	client.launch();
    	System.exit(0);
    }
    ...
    private void launch() {
        ...
        engineRequest.setRequestType(RequestType.ShiftSwapEligibility);
        ...
        requestData.setRequestType(RequestType.OpenShiftEligibility);
        requestData.setScheduleId(SCHEDULE_ID + now);
        requestData.setOriginatingAppServerId(ORIGIN_ID);

		String jsonEmployeeSchedule = "";
		try {
			jsonEmployeeSchedule = FileUtils.readFileToString(new File("data/input/ScheduleShiftSwapEligibilityInput.json"));
                ...
                if (EngineResponseType.Acknowledge.equals(engineResponse.getEngineResponseType())) {
                	...
                	if (completion == ScheduleCompletion.OK) {
                        ...
                        if (responseData != null && responseData.getResponseData() != null) {
                        	ShiftSwapEligibilityResultDto responseObj = EmlogisUtils.fromJsonString(
                                    responseData.getResponseData(), ShiftSwapEligibilityResultDto.class);
                        	
						    FileUtils.writeStringToFile(new File("data/output/ScheduleShiftSwapEligibilityResult.json"), responseData.getResponseData());
                        	...
                        	watch.stop();
                        	logger.info("Full time to execute schedule eligibility process: " + watch.getTime() + " ms");
                        }
                	} else {
