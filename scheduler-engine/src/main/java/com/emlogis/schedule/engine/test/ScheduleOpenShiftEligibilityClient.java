...
public class ScheduleOpenShiftEligibilityClient {

    private final static Logger logger = Logger.getLogger(ScheduleOpenShiftEligibilityClient.class);
    ...
    public static void main(String[] args) {
    	ScheduleOpenShiftEligibilityClient client = new ScheduleOpenShiftEligibilityClient();
    	client.launch();
    	System.exit(0);
    }
    ...
    private void launch() {
        ...
        engineRequest.setRequestType(RequestType.OpenShiftEligibility);
		...
		try {
			jsonEmployeeSchedule = FileUtils.readFileToString(new File("data/input/ScheduleOpenShiftEligibilityInput.json"));
                ...
                if (EngineResponseType.Acknowledge.equals(engineResponse.getEngineResponseType())) {
                	...
                	if (completion == ScheduleCompletion.OK) {
                        ...
                        if (responseData != null && responseData.getResponseData() != null) {
							...
							FileUtils.writeStringToFile(new File("data/output/OpenShiftEligibilityResult.json"), responseData.getResponseData());
